...
st.caption("Audited: Hard Stop Validation | DXF Detailing | Estimate | 🤖 AI Auto-Design Optimizer")
...
st.caption("Initial stiffness sizes. Used as fallback if AI is disabled.")
...
ai_auto_design = st.sidebar.toggle("🤖 Enable AI Auto-Design (Cost Optimizer)", value=False, help="Iteratively sizes members to find the cheapest section that passes IS 456 constraints.")
...
if st.button("🚀 Execute Analysis, Generate CAD/PDF & Estimates", type="primary", width="stretch"):
    ...
    with st.spinner("Solving Matrix, Running AI Optimization, Code Checks & Building CAD Files..."):
        ndof = len(nodes) * 6
        ...
        for el in elements:
            ...
            if ai_auto_design:
                ...
                for bw, hw in catalog:
                    b_m, h_m = bw/1000.0, hw/1000.0
                    if el['type'] == 'Beam':
                        req_bot, req_top, sv_mm, stat = design_beam_is456(el['L'], b_m, h_m, Mu_pos_max, Mu_neg_max, shear, torsion_max, fck, fy)
                        if stat == "Safe":
                            conc_vol = b_m * h_m * 1.0 
                            steel_wt = ((req_bot + req_top) / 1e6) * 7850 * 1.0
                            cost = (conc_vol * rate_conc_mat) + (steel_wt * rate_steel_mat)
                            if cost < best_cost:
                                best_cost, best_size, best_design = cost, f"{bw}x{hw}", (req_bot, req_top, sv_mm, stat)
                    else:
                        req_ast, sv_mm, stat = design_column_is456(b_m, h_m, axial, max(Mu_neg_max, Mu_pos_max), shear, torsion_max, fck, fy)
                        if stat == "Safe":
                            conc_vol = b_m * h_m * 1.0
                            steel_wt = (req_ast / 1e6) * 7850 * 1.0
                            cost = (conc_vol * rate_conc_mat) + (steel_wt * rate_steel_mat)
                            if cost < best_cost:
                                best_cost, best_size, best_design = cost, f"{bw}x{hw}", (req_ast, sv_mm, stat)
                                
                if best_design:
                    el['size'] = best_size  # Apply optimized size
            
            # --- FINAL SIZING AND REBAR GENERATION ---
            b_m, h_m = map(lambda x: float(x)/1000.0, el['size'].split('x'))
            if el['type'] == 'Beam':
                if ai_auto_design and best_design:
                    req_ast_bot, req_ast_top, sv_mm, stat = best_design
                else:
                ...
                if ai_auto_design and best_design:
                    req_ast, sv_mm, stat = best_design
                else:
                    req_ast, sv_mm, stat = design_column_is456(b_m, h_m, axial, max(Mu_neg_max, Mu_pos_max), shear, torsion_max, fck, fy)
        ...
        for idx, row in floors_df.iterrows():
            ...
            for col in f_cols:
                col_b_m, col_h_m = map(lambda val: float(val)/1000.0, col['size'].split('x'))
            ...
            for beam in f_beams:
                beam_b_m, beam_h_m = map(lambda val: float(val)/1000.0, beam['size'].split('x'))
